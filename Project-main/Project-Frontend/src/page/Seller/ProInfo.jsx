            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1.25rem 2rem 0' }}>
                ...
                <div style={{ display: 'flex', gap: '1.5rem', marginTop: '1.5rem', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ width: '360px', flexShrink: 0, position: 'sticky', top: '1rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                            <h3 style={{ margin: '0 0 1rem', fontSize: '16px', fontWeight: '600', color: '#1a1a1a' }}>ข้อมูลผู้ขาย</h3>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#001a4f', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', fontSize: '20px', flexShrink: 0 }}>
                                    {property.sellerName ? property.sellerName.charAt(0).toUpperCase() : '?'}
                                </div>
                                <div>
                                    <p style={{ margin: 0, fontWeight: '600', fontSize: '15px', color: '#1a1a1a' }}>{property.sellerName || 'ไม่ระบุชื่อ'}</p>
                                    <p style={{ margin: 0, fontSize: '13px', color: '#888' }}>ผู้ขาย</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setShowContact(true)}
                                style={{ width: '100%', padding: '10px', background: '#001a4f', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600', fontSize: '15px', cursor: 'pointer' }}
                            >
                                ติดต่อผู้ขาย
                            </button>
                        </div>
                        ...
                        {showContact && (
                            ...
                            >
                                <div
                                    ...
                                    style={{ background: 'white', borderRadius: '20px', padding: '2rem', width: '340px', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', position: 'relative' }}
                                >
                                    <button
                                        onClick={() => setShowContact(false)}
                                        style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#888', lineHeight: 1 }}
                                    >✕</button>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                                        <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: '#001a4f', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', fontSize: '22px', flexShrink: 0 }}>
                                            {property.sellerName ? property.sellerName.charAt(0).toUpperCase() : '?'}
                                        </div>
                                        <div>
                                            <p style={{ margin: 0, fontWeight: '700', fontSize: '16px', color: '#1a1a1a' }}>{property.sellerName || 'ไม่ระบุชื่อ'}</p>
                                            <p style={{ margin: 0, fontSize: '13px', color: '#888' }}>ผู้ขาย</p>
                                        </div>
                                    </div>
                                    <h4 style={{ margin: '0 0 1rem', fontSize: '15px', fontWeight: '600', color: '#333' }}>ช่องทางติดต่อ</h4>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                        {property.sellerPhone ? (
                                            <a href={`tel:${property.sellerPhone}`} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', background: '#f0f4ff', borderRadius: '12px', color: '#001a4f', textDecoration: 'none', fontWeight: '600', fontSize: '15px' }}>
                                                <span style={{ fontSize: '20px' }}>📞</span>
                                                {property.sellerPhone}
                                            </a>
                                        ) : null}
                                        {property.sellerEmail ? (
                                            <a href={`mailto:${property.sellerEmail}`} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', background: '#f0f4ff', borderRadius: '12px', color: '#001a4f', textDecoration: 'none', fontWeight: '600', fontSize: '15px' }}>
                                                <span style={{ fontSize: '20px' }}>✉️</span>
                                                {property.sellerEmail}
                                            </a>
                                        ) : null}
                                        {!property.sellerPhone && !property.sellerEmail && (
                                            <p style={{ margin: 0, color: '#aaa', fontSize: '14px', textAlign: 'center' }}>ไม่มีข้อมูลติดต่อ</p>
                                        )}
                                    </div>
                                </div>
                            </div>
